...
int mbedtls_net_send(void *ctx,const unsigned char *buffer, size_t len) {
	...
	if (len > ES_WIFI_PAYLOAD_SIZE)
	{
		...
		{
			if (c+ES_WIFI_PAYLOAD_SIZE>len)
				ret = WIFI_SendData(0, (uint8_t*)(buffer+c), (uint16_t)(len-c), &wrotelen, timeout);
			else
				ret = WIFI_SendData(0, (uint8_t*)(buffer+c), ES_WIFI_PAYLOAD_SIZE, &wrotelen, timeout);
			c = c + wrotelen;
			wrotelen = 0;
		} while (len>c && ret==0);
	}
	...
	{
		ret = WIFI_SendData(0, (uint8_t*)buffer, (uint16_t)len, &wrotelen, timeout);
		c = wrotelen;
	}
	if (ret == WIFI_STATUS_OK)
	{
